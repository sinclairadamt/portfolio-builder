<script>
  ...
  let openSection = $state(readStoredOpenSection())
  ...
  function readStoredOpenSection() {
    try {
      const stored = localStorage.getItem(OPEN_SECTION_STORAGE_KEY)
      return SECTIONS.includes(stored) ? stored : 'General'
    } catch {
      return 'General'
    }
  }
  ...
  $effect(() => {
    try {
      localStorage.setItem(OPEN_SECTION_STORAGE_KEY, openSection ?? '')
    } catch {
      // Same as above -- non-fatal.
    }
  })

  // Accordion: opening one section closes whichever was open, and
  // re-clicking the open section closes it (so every section can collapse,
  // keeping scroll length under control as more options get added).
      ...
      <div class="accordion-section">
        ...
        {#if openSection === 'Images'}
          <div class="section-body" transition:slide={{ duration: 180 }}>
            ...
            <label for="image-corner-radius">Corner Radius</label>
            <input
              id="image-corner-radius"
              type="number"
              min="0"
              max="40"
              value={settings.imageCornerRadius ?? 8}
              oninput={(e) => {
                const value = Number(e.target.value)
                if (!Number.isInteger(value) || value < 0 || value > 40) return
                settings.imageCornerRadius = value
                update()
              }}
            />
          </div>
...
<style>
  ...
  .accordion-section:not(:first-child) .section-heading {
